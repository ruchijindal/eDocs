...
<%@page import="com.sun.accessibility.internal.resources.accessibility_es"%>
...
<%@page import="java.util.logging.XMLFormatter"%>
...
<%@page import="java.util.*,
        ...
        %>
<%
            String userroletest = (String) session.getAttribute("userrole");
            if (userroletest.equals("null")) {

                //            System.out.println("gfdgdjkgdgdkhhkhkdhf gdkfg");
...
<%!    //FEATURES
   ...
   String userRight="" ;
    ...
    private static final String RESTRICT_PATH = "C:\\CODE;C:\\"; //Win32: Case important!!
    //private static final String RESTRICT_PATH = "/etc;/var;/home/smp;/home";
    private static final String REPOSITRY_PATH = "C:/Repository/";
     ...
     */
    private static final String[] FORBIDDEN_DRIVES = {"c:\\", "E:\\", "F:\\", "G:\\", "D:\\"};
    /**
    ...
    static String dir2linkdir(String dir, String browserLink, int sortMode) {
        ...
        StringBuffer buf = new StringBuffer();
        boolean check = false;
        //  System.out.println(f.getParentFile()+"file absolute path "+f.getAbsolutePath());
        while (f.getParentFile() != null) {
            if (f.canRead()) {
                String encPath = URLEncoder.encode(f.getAbsolutePath());
                //  System.out.println(File.separator+" current file name is ********** "+f.getName());
                if (check) {
                    f = f.getParentFile();
                    continue;
                }
                if (f.getName().equals("smp")) {
                    check = true;
                    continue;
                }
                //   System.out.println(f.getParent()+"  browser link "+browserLink+" sort mode "+sortMode+" fnmae "+f.getName());
                buf.insert(0, "<a href=\"" + browserLink + "?sort=" + sortMode + "&amp;dir="
                        + encPath + "\">" + conv2Html(f.getName()) + File.separator + "</a>");


            } else {
                //  System.out.println("file name in else "+f.getName());
        ...
        // System.out.println(buf.toString()+" and repository is "+REPOSITRY_PATH+".............................."+dir);
...
<%
            ...
            userRight=(String) session.getAttribute("userRight");
            
//request.setAttribute("dir", "/home/smp/Repositry");
...
<% 		} // View file
                ...
                System.out.println("file parents is " + f.getParent());
                ...
                if (notAllowedFile != null) {
                    ...
                    response.setHeader("Content-Disposition", "attachment;filename=\"eDocs.zip\"");
                    out.clearBuffer();
                    ...
                    System.out.println("Downloaded as Zip");
                    ...
                    if (logFile.exists()) {
                        ...
                        try {
                            ...
                            if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                childElement.setAttribute("status", String.valueOf(filecount));
                                String temp1 = filecount + " Files Download as Zip   From " + request.getRemoteAddr();
                    ...
                    if (logFile.exists()) {
                        ...
                        try {
                            ...
                            if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                Node childNode = doc.createElement("message");
                                Element childElement = (Element) childNode;
                                childElement.setAttribute("status", "d");
                                temp1 += " Downloaded   From " + request.getRemoteAddr();
                                childNode.setTextContent(temp1);
                                newNode.appendChild(childNode);
                                logNode.appendChild(newNode);
                                Transformer transformer = TransformerFactory.newInstance().newTransformer();
            ...
            System.out.println("file parents is " + request.getAttribute("dir"));
            if (request.getAttribute("dir") == null) {
                String path = (String) session.getAttribute("repositoryPath");//"/home/smp/Repository";
...
<body>
    ...
    Time: <%=info.getTimeElapsed()%> from <%=info.getTimeEstimated()%>
                                    ...
                                    Logger log = Logger.getLogger(session.getAttribute("userid").toString().trim());
                                    log.info(fname + "is Uploaded from " + request.getRemoteAddr());
                                } else {
                                    UploadMonitor.remove(fname);
%>
</head>
                            ...
                            if (logFile.exists()) {
                                ...
                                try {
                                    ...
                                    if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                        ...
                                        temp1 += " Uploaded   From " + request.getRemoteAddr();
                                        childNode.setTextContent(temp1);
                                        newNode.appendChild(childNode);
                                        logNode.appendChild(newNode);
                                        Transformer transformer = TransformerFactory.newInstance().newTransformer();
...
<%
                    ...
                    if (logFile.exists()) {
                        ...
                        try {
                            ...
                            if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                temp1 += f.getName() + " Deleted   From " + request.getRemoteAddr();
                                childNode.setTextContent(temp1);
                                newNode.appendChild(childNode);
                                logNode.appendChild(newNode);
                                Transformer transformer = TransformerFactory.newInstance().newTransformer();
                ...
                if (!isAllowed(new File(new_dir), true)) {
                    ...
                    if (logFile.exists()) {
                        ...
                        try {
                            ...
                            if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                Node childNode = doc.createElement("message");
                                String temp1 = "Directory " + new_dir + " Created From " + request.getRemoteAddr();
                                childNode.setTextContent(temp1);
                                newNode.appendChild(childNode);
                                logNode.appendChild(newNode);
                                Transformer transformer = TransformerFactory.newInstance().newTransformer();
                    ...
                    request.setAttribute("error", "Creation of directory " + new_dir + " failed(Folder Name Empty)");
                }
                ...
                if (!isAllowed(new File(new_file), true)) {
                    ...
                    if (new File(new_file).createNewFile()) {
                        System.out.println("new File Create entry in the log");
                        ...
                        if (logFile.exists()) {
                            try {
                                ...
                                if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                    ...
                                    Node childNode = doc.createElement("message");
                                    String temp1 = "File " + new_file + " Created from " + request.getRemoteAddr();
                                    childNode.setTextContent(temp1);
                                    newNode.appendChild(childNode);
                                    logNode.appendChild(newNode);
                                    Transformer transformer = TransformerFactory.newInstance().newTransformer();
                ...
                //     Here we assume that expandFileList builds v from top-bottom, starting with the dirs
                    ...
                    if (!isAllowed(f, true)) {
                        ...
                        if (!f.canWrite() || !f.renameTo(new File(new_file.trim()))) {
                            ...
                            File logFile = new File(logPath + session.getAttribute("userid").toString().trim() + ".xml");
                            if (logFile.exists()) {
                                ...
                                try {
                                    ...
                                    if (logNode.getNodeType() == Node.ELEMENT_NODE) {
                                        ...
                                        Node childNode = doc.createElement("message");
                                        String temp1 = "File " + ((File) v.get(0)).getName() + " renamed to " + new_file + "  From " + request.getRemoteAddr();
                                        childNode.setTextContent(temp1);
                                        newNode.appendChild(childNode);
                                        logNode.appendChild(newNode);
                                        Transformer transformer = TransformerFactory.newInstance().newTransformer();
                ...
                <div class="content-box-content">
                    ...
                    <%
                                    ...
                                    if (request.getAttribute("message") != null) {
                                        ...
                                        out.println("<div class=\"notification information png_bg\">\n");
                                        out.println("<a href=\"#\" class=\"close\"><img src=\"/SMPDocMan/resources/images/icons/cross_grey_small.png\" title=\"Close this notification\" alt=\"close\" /></a>\n");
                                        out.println("                                <div>\n");
                                    ...
                                    if (request.getAttribute("error") != null) {
                                        out.println("<div class=\"notification error1 png_bg\">\n");
                                        out.println("<a href=\"#\" class=\"close\"><img src=\"/SMPDocMan/resources/images/icons/cross_grey_small.png\" title=\"Close this notification\" alt=\"close\" /></a>\n");
                                        out.println("                                <div>\n");
                    ...
                    <form class="formular" action="<%= browser_name%>" method="Post" name="FileList">
                        ...
                        <table id="filetable" class="filelist" cellspacing="1px" cellpadding="0px" >
                            <%
                                                                    ...
                                                                    out.println("<th align=\"LEFT\" >&nbsp;<a href=\" \">Action</a></th></tr>");
                                                                    char trenner = File.separatorChar;
                                                                    // Output the Root-Dirs, without FORBIDDEN_DRIVES
                                                                    File[] entry = File.listRoots();
                                                                    System.out.println("size of entry is " + entry.length);
                                                                    System.out.println(System.getProperty("os.arch") + "Os anme " + System.getProperty("os.name") + "file sperator " + System.getProperty("file.separator"));
                                                                    Map<String, String> maps = System.getenv();
                                                                    Collection<String> ol = maps.values();

                                                                    for (int i = 0; i < entry.length; i++) {
                                                                        boolean forbidden = false;
                                                                        System.out.println("absoulte pathj " + entry[i].getAbsolutePath());
                                                                        if (System.getProperty("os.name").equalsIgnoreCase("Linux")) {
                                                                            System.out.println("I am in lunux code");
                                                                            ...
                                                                            if (entry[i].getAbsolutePath().equals("C:\\")) {
                                                                                System.out.println("file windows ");
                                                                               forbidden=false;
                                                                            }else{
                                                                                forbidden=true ;
                                                                                }
                                                                        ...
                                                                        System.out.println("forbidden is "+forbidden);
                                                                        if (!forbidden) {
                                                                            out.println("<tr class=\"mouseout\" onmouseover=\"this.className='mousein'\""
                                                                                    + "onmouseout=\"this.className='mouseout'\">");
                                                                            out.println("<td>&nbsp;</td><td align=left >");
                                                                            String name = session.getAttribute("repositoryPath").toString();
                                                                            System.out.println("name value is " + name);
                                                                            String buf = entry[i].getAbsolutePath() + session.getAttribute("repositoryPath").toString();
                                                                            String gda = browser_name;
                                                                            System.out.println(entry[i].getAbsolutePath() + " gda path is " + gda);
                                                                    ...
                                                                    System.out.println(f.getAbsolutePath() + " parent of current directory is " + f.getParent());
                                                                    String repPath = session.getAttribute("repositoryPath").toString();
                                                                    System.out.println(f.getAbsolutePath() +""+System.getProperty("file.separator")+ "      Repository path is   " + repPath);
                                                                    if (((f.getAbsolutePath() +System.getProperty("file.separator")).equals(repPath))) {
                        ...
                        userRight=(String)session.getAttribute("accessRight");
                        System.out.println("Acess Right In file Manager111111 "+userRight);

                        if(userRight.equals("rw")||userRight.equals("mn")){ %>
